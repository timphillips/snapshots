const init = (() => {
  ...
  const { map, filter, scan, startWith, combineLatest, distinctUntilChanged } = rxjs.operators;

  /**
   * Returns a new array that contains the elements of the passed-in array in a random order.
   *
  ...
  function createProgressStream(scrollStream, progressLimit) {
  ...
  function createControlsOpacityStream(progressStream) {
  ...
  function createIntroOpacityStream(progressStream) {
    return progressStream.pipe(
  ...
  function createOutroOpacityStream(progressStream, progressLimit) {
    return progressStream.pipe(
  ...
  function createImageStreams(progressStream, images, stepsPerImage) {
  ...
  function createPercentWithinImageStream(progressStream, stepsPerImage) {
    return progressStream.pipe(
      map(progress => {
  ...
  function createImageOpacityStream(progressStream, percentWithinImageStream, progressLimit) {
    return progressStream.pipe(
  ...
  function createImageBlurStream(percentWithinImageStream) {
  ...
  function createImageHeightStream(percentWithinImageStream, zoomStream) {
  ...
  function createImageSepiaStream(sepiaStream) {
  ...
  function init() {
    ...
    const stagingElement = window.document.getElementById("staging");
    ...
    nextImageStream.subscribe(image => (stagingElement.src = image));
